...
GOALS_FILE = Path("verification metadata")


def _write_goals_status(results: list[VerifyResult]) -> None:
    # verification metadata's status field is only ever written here, by an actual
    # verifier run - never by hand, per architecture notes §9. round-trips through
