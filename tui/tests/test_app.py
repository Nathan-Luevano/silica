...
async def test_every_pane_renders_against_real_shaped_artifacts(full_artifacts: Path) -> None:
    app = ScopeApp(full_artifacts, full_artifacts.parent / "verification metadata")
...
async def test_reload_refreshes_every_pane_not_just_the_overview(
    ...
    app = ScopeApp(live, tmp_path / "live" / "verification metadata")
    async with app.run_test(size=(140, 40)) as pilot:
        await pilot.press("5")
        ...
        shutil.copy(full_artifacts.parent / "verification metadata", tmp_path / "live" / "verification metadata")
        await pilot.press("r")
