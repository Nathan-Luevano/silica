...
def goals_table(session: Session) -> RenderableType:
    if not session.goals:
        return missing_panel(
            ...
            session.goals_error or "verification metadata not found next to the artifacts root",
        )
    table = Table(box=SIMPLE, expand=True, pad_edge=False, header_style=DIM)
...
# what pysilica's verifier actually writes into verification metadata is "passing" /
